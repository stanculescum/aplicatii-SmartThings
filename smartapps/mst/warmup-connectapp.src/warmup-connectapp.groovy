 ...
 *  Copyright 2019
 *
 *  This application is based on Warmup (Connect) by Alex Lee Yuk Cheung. Many thanks.
 *
...
def apiURL() { 
	return "https://api.warmup.com/apps/app/v1" 
}
	...
	if (username == null || username == '' || password == null || password == '') {
		...
		return dynamicPage(name: "firstPage", title: "", install: true, uninstall: true) {
			...
			if (stateTokenPresent()) {           	
				section ("Choose your Warmup 4IE devices:") {
...
def selectDevicePAGE() {
	updateLocations()
	dynamicPage(name: "selectDevicePAGE", title: "Warmup 4IE Devices", uninstall: false, install: false) {
		section { 
			headerSECTION()
		}
		if (devicesSelected() == null) {
			section("Select your Location:") {
				input "selectedLocation", "enum", image: "https://raw.githubusercontent.com/stanculescum/aplicatii-smarthome/master/pictures/warmup-location.png", required:true, title:"Select a Location \n(${state.warmupLocations.size() ?: 0} found)", multiple:false, options:state.warmupLocations, submitOnChange:true
			}
		}
		else {
			section("Your location:") {
				paragraph (image: "https://raw.githubusercontent.com/stanculescum/aplicatii-smarthome/master/pictures/warmup-location.png", "Location: ${state.warmupLocations[selectedLocation]}\n(Remove all devices to change location)")
			}
		}
		if (selectedLocation) {
			updateDevices() 
			section("Select your Warmup 4IE devices:") {
				input "selectedWarmup4IEs", "enum", image: "https://raw.githubusercontent.com/stanculescum/aplicatii-smarthome/master/pictures/warmup-4ie.png", required:true, title:"Select Warmup 4IE Devices \n(${state.warmup4IEDevices.size() ?: 0} found)", multiple:true, options:state.warmup4IEDevices, submitOnChange:true
			}
		}
	}
}
...
def headerSECTION() {
	return paragraph (image: "https://raw.githubusercontent.com/stanculescum/aplicatii-smarthome/master/pictures/warmup-icon.png", "${textVersion()}")
}
...
def addWarmup4IE() {
	...
	selectedWarmup4IEs.each { device ->
			...
			childDevice = addChildDevice(app.namespace, "Warmup 4IE (childDevice)", "$device", null, data)
