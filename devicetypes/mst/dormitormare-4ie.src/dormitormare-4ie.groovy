...
metadata {
	definition (name: "DormitorMare 4IE", namespace: "mST", author: "Mihail stanculescu", ocfDeviceType: "oic.d.thermostat", mnmn: "SmartThings", vid: "SmartThings-smartthings-Z-Wave_Thermostat") {
		capability "Actuator"
		capability "Polling"
		capability "Refresh"
...
def poll() {
    ...
    def temperature = String.format("%2.1f",(room.currentTemp[3] as BigDecimal)/ 10)
    sendEvent(name: 'temperature', value: temperature, unit: "C", state: "heat")
    
    def heatingSetpoint = String.format("%2.1f",(room.targetTemp[3] as BigDecimal) / 10)
    ...
    def averageAir = String.format("%2.1f",(room.airTemp[3] as BigDecimal) / 10)
