/**
 *  Warmup 4ie (childDevice)
 *
 *  Copyright 2019
 *
 *	This application is based on Warmup 4IE by Alex Lee Yuk Cheung. Many thanks.
 *
 ...
 *
 *	VERSION HISTORY
 *  10.10.2019	v1.0 - Initial Release
 */
...
metadata {
	definition (name: "Warmup 4IE (childDevice)", namespace: "mST", author: "Mihail stanculescu", ocfDeviceType: "oic.d.thermostat", mnmn: "SmartThings", vid: "SmartThings-smartthings-Z-Wave_Thermostat") {
		capability "Actuator"
		capability "Polling"
		capability "Refresh"
...
def poll() {
    ...
    def temperature = String.format("%2.1f",(room.currentTemp[nbroom] as BigDecimal)/ 10)
    sendEvent(name: 'temperature', value: temperature, unit: "C", state: "heat")
    
    def heatingSetpoint = String.format("%2.1f",(room.targetTemp[nbroom] as BigDecimal) / 10)
    ...
    def averageAir = String.format("%2.1f",(room.airTemp[nbroom] as BigDecimal) / 10)
